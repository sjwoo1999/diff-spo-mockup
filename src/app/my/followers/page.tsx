...
export default function FollowersPage() {
  const router = useRouter();
  const followers = database.user.followers || [];
      ...
      <div className="relative w-full max-w-xl flex flex-col">
        <SafeAreaWrapper className="min-h-screen bg-white pb-24 flex flex-col">
          <main className="flex-1 flex flex-col w-full max-w-xl mx-auto px-4 pt-6">
            ...
            <h1 className="text-2xl font-bold mb-4">팔로워</h1>
            <div className="flex flex-col gap-4">
              {followers.length === 0 ? (
                <Card className="text-center py-8 text-gray-500">아직 팔로워가 없습니다</Card>
              ) : (
                followers.map(fid => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <button className="px-4 py-1.5 bg-primary text-white text-sm rounded-full shadow hover:bg-primary-dark transition" onClick={e => {e.stopPropagation();}}>팔로우</button>
                    </Card>
                  );
                })
              )}
